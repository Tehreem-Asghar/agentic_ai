from agents import Agent , Runner , ModelSettings , AsyncOpenAI , OpenAIChatCompletionsModel , set_default_openai_client , set_tracing_disabled , function_tool
...
pdf_file_path = "profile.pdf"
pdf_pages, pdf_doc_ids = load_and_split_multiple_pdfs([pdf_file_path])


document_content = [page.page_content for page in pdf_pages]
document_ids = pdf_doc_ids

# print(document_content[0][:300])  # print first 300 chars of page 1
...
print(f"✅ Added {len(pdf_pages)} PDF pages to the pdf_knowledge_base.")
...
def answer_from_pdf(query: str) -> str:
    ...
    # print("[Debug] Query Embedding Vector:", q_vector)


    res = collection.query(query_embeddings=[q_vector], n_results=1, include=["documents"])
    # print("[Debug] Retrieved Result:", res)

    if res and res.get("documents") and res["documents"][0]:
        top_doc = res["documents"][0][0]
        # print("[Debug] Top Document Context:", top_doc)
...
async def main():
    answer = await Runner.run(agent,"What is Tehreem Asghar's top skills" , run_config=config)
    print("Answer from agent : \n",answer.final_output)
    return answer.final_output

    # answer = await answer_from_pdf("What is Tehreem Asghar's top skills?")  
